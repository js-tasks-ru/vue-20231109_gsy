<template>
  <UiCalendarView>
    <template #events="{date}">
      <div v-for="holiday in getDayHolidays(date)" :key="holiday" class="holiday">
        {{ holiday }}
      </div>
    </template>
  </UiCalendarView>
...
<script setup>
  ...
  const internationalHolidays = reactive([
   ...
   * @returns {string[][]}
   */
  const internationalHolidaysMap = computed(() => {
    const result = Array.from(Array(12), () => ({}));
    for (const { date, month, holiday } of internationalHolidays) {
  ...
  /**
   * Получить набор праздников на выбранную дату.
   * @param {number} time Временная метка даты.
   * @returns {string[]} Праздники на одну дату.
   */
  function getDayHolidays(time) {
    let date = new Date(time);
    let [m, d] = [date.getMonth(), date.getDate()];
    return internationalHolidaysMap.value[m]?.[d];
  }
</script>
